			...
			<div className="first_menu-configurator-nav-title">
				...
				<nav className='first_menu-configurator-nav'>
					<ul className='sub-menu menu-depth-1'>
						...
						<li key={i} className={`sub-menu--li ${title[0]}`}>{title[0].includes('Mark') ? title[0] : <a href={title[1]} className='sub-menu--a'>{title[0]}</a>}{title[0].includes('Mark') ? <img src= {Caret} alt="caret" className={`caret caret--${i}` } onClick={clicCaret}/> : ''}</li>
						))}
					</ul>
				</nav>
			</div>
